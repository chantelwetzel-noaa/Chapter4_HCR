...
png(filename = "Depletion_Dist_95SI_alt.png", width = 10, height = 8.5, units = 'in', res = 256)

par(mfrow = c(2, 3), mar = c(0.7,0.5,1.2,1), oma = c(4,4,4,5))
...
for (a in 1:length(steep.vec)) {
	...
	polygon(xx, yy, col = "deepskyblue", lty = 0)		
	...
	xx = c(out$x, rev(out$x)); yy = c(out$y + offset, rev(rep(offset, length(out$y))))
	polygon(xx, yy, col = "dodgerblue3", lty = 0)
...
png(filename = "Tradeoffs_msy_alt.png", width = 10, height = 8.5, units = 'in', res = 256)

pch.vec = 21:24
pch.col = c('red', 'green', 'blue') #pch.col = c(1,  "grey50", "white")
par(mfrow = c(2, 3), mar = c(3,4,3,3), oma = c(2,2,1,0))
letter.cex = 1.6; axis.cex = 1.6; label.cex = 1.5; pch.cex = 2; main.cex = 1
...
points(0.03, msy.vec[1], pch = pch.vec[1], bg = pch.col[2], cex = 1.7)
abline(h = msy.vec[2], lty = 2, col = 'grey70')
points(0.03, msy.vec[2], pch = pch.vec[1], bg = pch.col[1], cex = 1.7)
abline(h = msy.vec[3], lty = 2, col = 'grey70')
points(0.03, msy.vec[3], pch = pch.vec[1], bg = pch.col[3], cex = 1.7)
box()
for(a in 1:3){
...
for(a in 1:3){
	...
	mtext(side = 2, "Average annual variation", line = 3, cex = label.cex)
	mtext(side = 1, part2, line = 3.5, cex = label.cex)
	for(b in 2:length(hcr.vec)){
...
legend('topright', legend = labels2[1:3], pch = rep(16,3), col = pch.col, bty = 'n', cex = letter.cex + 0.25)
legend('topright', legend = labels2[1:3], pch = rep(21,3), col = rep(1,3), bty = 'n', cex = letter.cex + 0.25)
...
legend('topright', pch = pch.vec, legend = c("20-5", "25-5", "30-10", "40-10"), bty = 'n', cex = letter.cex + 0.25)
...
abline(h = msy.vec[1], lty = 2, col = 'grey70')
points(0.03, msy.vec[1], pch = pch.vec[1], bg = pch.col[1], cex = 1.7)
points(0.05, msy.vec[1], pch = pch.vec[1], bg = pch.col[2], cex = 1.7)
points(0.07, msy.vec[1], pch = pch.vec[1], bg = pch.col[3], cex = 1.7)
box()
for(a in 4:6){
	lines(target10[a,], ave.catch[a,], lty = 2)
...
legend('topright', legend = labels2[4:6], pch = rep(16,3), col = pch.col, bty = 'n', cex = letter.cex + 0.25)
legend('topright', legend = labels2[4:6], pch = rep(21,3), col = rep(1,3), bty = 'n', cex = letter.cex + 0.25)
...
for(a in 4:6){
	...
	axis(side = 2, las = 1,  at = c(0,5,10,15), cex.axis = axis.cex)
	mtext(side = 2, "Average annual variation", line = 3, cex = label.cex)
	mtext(side = 1, "Average catch", line = 3.5, cex = label.cex)
	for(b in 2:length(hcr.vec)){
